...
export const api = {
  ...
  admin: {
    ...
    deleteUser: (userId: number) => del(`/api/admin/users/${userId}`),
    setUserPassword: (userId: number, password: string) =>
      put(`/api/admin/users/${userId}/password`, { password }),
    setUserProjects: (userId: number, projectIds: string[]) =>
      put(`/api/admin/users/${userId}/projects`, { projectIds }),
  },

  auth: {
    status: (init?: RequestInit) => fetch('/api/auth/status', init),
    ...
    user: (options?: ApiRequestOptions) => get('/api/auth/user', options),
  },
