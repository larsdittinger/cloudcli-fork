...
test('a refreshed token older than the one sent is ignored (replayed from browser cache)', async () => {
  // On a 304 the browser hands back the *cached* headers, so X-Refreshed-Token
  // can carry a token from days ago. Storing it would swap the fresh login
  // token for a stale one and, once that one expired, end the session.
  const now = Math.floor(Date.now() / 1000);
  const current = makeToken({ iat: now - 60, exp: now + 6 * 24 * 3600 });
  const stale = makeToken({ iat: now - 3 * 24 * 3600, exp: now + 3 * 24 * 3600 });
...
test('an auth error on a request sent without a token leaves the session alone', async () => {
  // Providers mounted before login fire requests with no token; their 401s say
  // nothing about the session the user is about to get.
  let expiries = 0;
  const onExpired = () => {
    expiries += 1;
  };
  ...
  await (await loadFetch(false))('/api/plugins');
