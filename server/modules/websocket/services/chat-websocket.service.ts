...
async function handleChatSend(
  ...
  const resolved = resolveSendTarget(ws, user, data, dependencies, 'chat.send');
  if (!resolved) {
    return;
  }
...
async function handleChatEditSend(
  ...
  const resolved = resolveSendTarget(ws, user, data, dependencies, 'chat.edit-send');
  if (!resolved) {
    return;
  }
...
export function handleChatConnection(
  ...
  ws.on('message', async (rawMessage) => {
    try {
      ...
      switch (messageType) {
        case 'chat.edit-send':
          await handleChatEditSend(ws, user, userId, data, dependencies);
          return;
        case 'chat.send':
